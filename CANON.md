---
canon: true
stability: locked
last_reviewed: 2026-02-05
---

# Canon Index

This file is the canonical index for Quantum Error.

## Locked Canon

### Characters (Main Party)
- `characters/roster.md`
- `characters/party/clint.md`
- `characters/party/vanessa.md`
- `characters/party/rebekah.md`
- `characters/party/selene.md`
- `characters/party/wade.md`

### Classes (Main Party)
- `classes/paladin-of-the-system.md`
- `classes/quantum-sorceress.md`
- `classes/temporal-bard.md`
- `classes/veilstepper-rogue.md`
- `classes/cleric-of-healing.md`

## Evolving Canon

### Characters
- `characters/villains/eron-vosk.md`
- `characters/villains/dan.md`
- `characters/supporting/the-twins.md`
- `characters/supporting/amanda.md`
- `characters/supporting/celeste.md`

### Classes
- `classes/README.md`
- `classes/mind-mage.md`
- `classes/necromancer.md`

### Races
- `races/README.md`
- `races/nekara.md`
- `races/goblin.md`
- `races/troll.md`

### Magic
- `magic/overview.md`
- `magic/schools/system-magic.md`
- `magic/spells/ignite.md`
- `magic/spells/create-light.md`
- `magic/spells/create-water.md`
- `magic/spells/system-dispel.md`
- `magic/spells/chrono-resonance.md`

### Items
- `items/README.md`
- `items/equipment-framework.md`
- `items/weapons/katsuragi.md`
- `items/armor/aegis-of-decoherent-deflection.md`
- `items/accessories/fanny-pack-of-holding.md`

### Professions
- `professions/enchanting.md`

### Lore
- `lore/overview.md`
- `lore/timeline.md`
- `lore/locations/bay-area.md`
- `lore/organizations/iquantum.md`

### System
- `system/overview.md`
- `system/stats.md`
- `system/combat.md`
- `system/mana.md`
- `system/effects.md`
- `system/targeting.md`
- `system/ui-popups.md`
- `system/stat-progression.md`
- `system/mob-framework.md`

### Story
- `story/chapter-summaries.md`

### Meta
- `canon/status.md`
- `canon/glossary.md`
- `canon/style-guide.md`
